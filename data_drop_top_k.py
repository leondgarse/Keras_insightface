...
def data_drop_top_k(model, data_path, dataset_pickle_file_dest=None, deg_thresh=75, limit=0):
    cos_thresh = np.cos(np.pi * deg_thresh / 180)   # 0.25881904510252074

    image_names, image_classes, _, dataset_pickle_file_src = pre_process_folder(data_path)  # Reload from pickle file
    ...
    if dataset_pickle_file_dest is None:
        ...
        dataset_pickle_file_dest = src_name + "_topK{}_deg{}.pkl".format(top_k, deg_thresh)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--dataset_pickle_file_dest", type=str, default=None, help="Dest file path to save the processed dataset pickle")
    ...
    print(">>>> Output:", data_drop_top_k(args.model_file, args.data_path, args.dataset_pickle_file_dest, args.deg_thresh, args.limit))
elif __name__ == "__test__":
    model_file = "TT_mobilenet_topk_bs256.h5"
    data_path = "/datasets/faces_casia"
    dataset_pickle_file_dest = None
    deg_thresh = 75
    data_drop_top_k(model_file, dataset_pickle_file_src, dataset_pickle_file_dest, deg_thresh)
