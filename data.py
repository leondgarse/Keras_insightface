...
def pre_process_folder(data_path, image_names_reg=None, image_classes_rule=None):
    ...
    if os.path.exists(dest_pickle):
        ...
        print(">>>> preprocessed dataset backup saved to dest_pickle:", dest_pickle)
    classes = np.max(image_classes) + 1 if len(image_classes) > 0 else 0
    print(">>>> Loaded data image_names:", len(image_names), "image_classes:", len(image_classes), "embeddings:", len(embeddings), "classes:", classes)
...
def build_dataset_from_tfrecord(data_path, classes=-1, total_images=-1):
    ...
    if classes <= 0 or total_images <= 0:  # or set value like classes=85742, total_images=5822653 for MS1M_V3 skipping this step
        print(">>>> Reading entire dataset determining classes and total_images")
        labels = [int(tf.io.parse_single_example(example, feature_description)["label"]) for example in tqdm(ds.as_numpy_iterator())]
        classes, total_images = np.max(labels) + 1, len(total_images)
