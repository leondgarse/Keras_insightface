...
class Bneck(tf.keras.layers.Layer):
    def __init__(
        self, filters, expansion_filters, kernel_size, alpha=1.0, strides=(1, 1), use_se=False, activation=tf.nn.relu6, **kwargs
    ):
        ...
        self.expand_conv2d = tf.keras.layers.Conv2D(
            self.expansion_filters, 1, padding="same", use_bias=False, kernel_regularizer=tf.keras.regularizers.l2(1e-5)
        )
        ...
        self.se = SeBlock()
        self.project_conv2d = tf.keras.layers.Conv2D(
            self.filters, kernel_size=1, padding="same", use_bias=False, kernel_regularizer=tf.keras.regularizers.l2(1e-5)
        )
