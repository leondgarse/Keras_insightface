...
class Train:
    def __init__(
        ...
        callbacks = myCallbacks.basic_callbacks(checkpoint=save_path, lr=lr_base)
        self.callbacks = my_evals + callbacks
    ...
    def train(self, train_schedule, initial_epoch=0):
        for sch in train_schedule:
            ...
            if sch.get("centerloss", False):
                ...
                if sch["loss"].__class__.__name__ != losses.Center_loss.__name__ :
                    feature_dim = self.basic_model.output_shape[-1]
                    initial_file = self.basic_model.name + '_centers.npy'
                    logits_loss = sch["loss"]
                    center_loss = losses.Center_loss(self.classes, feature_dim=feature_dim, factor=1.0, initial_file=initial_file, logits_loss=logits_loss)
                    sch["loss"] = center_loss
                self.callbacks.insert(0, sch["loss"].save_centers_callback)
                self.model = keras.models.Model(self.model.inputs[0], keras.layers.concatenate([self.basic_model.outputs[0], self.model.outputs[-1]]))
                self.metrics = [sch["loss"].logits_accuracy]
            elif type == self.triplet:
                self.metrics = None
            else:
                self.metrics = ["accuracy"]
